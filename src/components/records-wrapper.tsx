        ...
        {folderContext?.currentFolder !== null ? (
          ...
          <NoRecordsAlert title="No folder selected">
            <div className="flex items-center">
              Please do select current folder using the{" "}
              <FoldersIcon className="flex ml-4 mr-4" /> icon above.
            </div>
          </NoRecordsAlert>
        )}
      </div>
      <div className="items-center justify-between gap-4 mt-4 text-sm text-center p-6">
        By using the application You accept Doctor Dok{" "}
        <Link className="underline hover:text-blue-500" href={termsUrl}>
          Terms of Service
        </Link>{" "}
        and <Link href={privacyUrl}>Privacy Policy</Link>.
      </div>
    </div>
  );
}
